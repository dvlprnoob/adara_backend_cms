...
# CMS - GET ALL TYPES
# =========================
# Admin melihat semua (global + user)
...
def update_type(type_id: int, payload: EmergencyTypeUpdate, user_id: int, db: Session = Depends(get_db)):
    item = db.query(EmergencyType).filter(EmergencyType.id == type_id).first()

    if not item:
        raise HTTPException(status_code=404, detail="Not found")

    # Kalau personal dan bukan owner -> forbidden
    if item.created_by and item.created_by != user_id:
...
# User hanya delete miliknya
@router.delete("/{type_id}")
def delete_type(type_id: int, user_id: int, db: Session = Depends(get_db)):
    item = db.query(EmergencyType).filter(EmergencyType.id == type_id).first()
...
# =========================
# TOGGLE ACTIVE (ADMIN MODERATION)
# =========================
